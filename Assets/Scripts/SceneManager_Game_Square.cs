...
//		   button selections
// ---------------------------------------------------------------------------------------
public class SceneManager_Game_Square : SceneManager_Base 
{
	// set via inspector
	...
	private BoardManagerSquare boardManager = null;
	...
	private string leaderboard1 = "CgkIgP7b-ckPEAIQEQ";
	private string leaderboard2 = "CgkIgP7b-ckPEAIQCQ";
	private string leaderboard3 = "CgkIgP7b-ckPEAIQEw";
	...
	void Start(){
		...
		PlayerPrefs.SetInt("played_square",++data.gameStatsSquare[0]);
	...
	void Update(){
		if (boardManager.state == GameState.GameOver){
			if(!sceneEnded){
				...
				// increase the games played count
				PlayerPrefs.SetInt("completed_square",++data.gameStatsSquare[1]);
				...
				if (index <= 3){
					...
					PlayerPrefs.SetInt(stats[index-1], ++data.gameStatsSquare[5-index]);
				}
				...
				if (index == 1){
					...
					for (int i = 4; i >= 0; i--){
						if (time < data.fastestTimesSquare[i] || data.fastestTimesSquare[i] == 0){
							data.fastestTimesSquare[i] = time;
							Array.Sort(data.fastestTimesSquare, CustomSort);

							PlayerPrefs.SetInt("fastest_square", data.fastestTimesSquare[0]);
							PlayerPrefs.SetInt("second_square", data.fastestTimesSquare[1]);
							PlayerPrefs.SetInt("third_square", data.fastestTimesSquare[2]);
							PlayerPrefs.SetInt("fourth_square", data.fastestTimesSquare[3]);
							PlayerPrefs.SetInt("fifth_square", data.fastestTimesSquare[4]);
				...
				if (wantsAchievements) {
					...
					if (index <= 1){
						...
						waitingLeaderboards.Add (new LeaderboardValue(leaderboard3, data.gameStatsSquare[4]));
